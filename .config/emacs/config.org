...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; use-package を straight.el でインストール
  (straight-use-package 'use-package)

  ;; デフォルトで :straight t を有効にする
...
#+begin_src emacs-lisp :tangle yes
  ;; direnv integration for buffer-local environment
  (use-package envrc
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; Show vertico in a floating frame
...
#+begin_src emacs-lisp :tangle yes
  ...
  (use-package corfu
    ...
    (setq corfu-auto-delay 0.2)   ; 補完までの遅延（秒）
    (setq corfu-auto-prefix 2)    ; 補完開始文字数
    (setq corfu-cycle t)          ; 候補をサイクル
    (setq corfu-preselect 'prompt)) ; 最初の候補を選択しない
  ...
  (use-package kind-icon
    :after corfu
    :config
    (require 'org)  ; svg-libがorg-plist-deleteを使うため
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;; Display eldoc in a floating box (like Neovim hover)
      ...
      (while (re-search-forward "```\\([a-zA-Z0-9_-]*\\)\n\\(\\(?:.*\n\\)*?\\)```" nil t)
        (let* ((lang (match-string 1))
               ...
               (mode (cond
                      ...
                      ((member lang '("nix")) 'nix-ts-mode)
                      ;; バグってるので無効
                      ;; ((member lang '("gleam")) 'gleam-ts-mode)
                      (t nil)))
...
#+begin_src emacs-lisp :tangle yes
    ;; treesit は Emacs 29+ に組み込まれているので use-package 不要
    ...
    (dolist (lang '(typescript tsx javascript json python go gomod css html yaml bash gleam))
...
#+begin_src emacs-lisp :tangle yes
  ...
  (setq claude-code-ide-terminal-backend 'vterm)
      ...
      (use-package claude-code-ide
        ...
        ;; claude-code-ideのバッファでevil-modeを無効化
        (with-eval-after-load 'evil
          ;; claude-code-ideの主要なモードでEmacsステート（evil無効）に設定
          (dolist (mode '(claude-code-ide-mode
                          claude-code-ide-chat-mode
                          claude-code-ide-terminal-mode))
